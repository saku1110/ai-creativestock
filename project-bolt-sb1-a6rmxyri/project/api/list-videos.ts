...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  try {
    ...
    const expires = Math.min(parseInt((req.query.expires as string) || '21600', 10) || 21600, 60 * 60 * 24 * 7);
    ...
    const entries = (data || []).filter((f: any) => /(mp4|webm)$/i.test((f.name || '').split('.').pop() || ''));
    ...
    const items = (
      await Promise.all(paths.map(async (p) => {
        ...
        const { data } = client.storage.from(bucket).getPublicUrl(p);
        return data?.publicUrl ? { path: p, url: data.publicUrl } : null;
      }))
    ).filter(Boolean);
