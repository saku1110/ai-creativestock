...
const Hero: React.FC<HeroProps> = ({ onAuthRequest, onPurchaseRequest }) => {
  ...
  const registerVideo = React.useCallback(
    (container: HTMLElement | null, video: HTMLVideoElement | null) => {
      ...
      if (observerRef.current) observerRef.current.observe(container);
      return () => {
        itemsRef.current.delete(container);
        if (observerRef.current) observerRef.current.unobserve(container);
      };
    },
    []
  );

  const handleVideoReady = React.useCallback((src: string) => {
    if (uniqueVideoCount === 0 || isReadyRef.current) return;
    const loaded = loadedVideosRef.current;
    if (!loaded.has(src)) {
      loaded.add(src);
      if (loaded.size >= uniqueVideoCount) setIsReady(true);
    }
  }, [uniqueVideoCount]);
  ...
  useEffect(() => { computeSpeed(); }, [doubledVideos.length]);
  ...
  return (
    <section className="relative py-16 sm:py-24 lg:py-32 overflow-hidden">
      ...
      <div className="absolute inset-0">
        ...
        <div className="absolute bottom-1/4 right-1/4 w-96 h-96 bg-purple-500/20 rounded-full blur-3xl animate-pulse" style={{ animationDelay: '1000ms' }}></div>
      </div>
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center">
          ...
          <div className="relative overflow-hidden py-8 mb-8">
            ...
            >
              <div className="flex items-center gap-3 sm:gap-4 lg:gap-6 px-4 sm:px-6 lg:px-8" data-hero-track>
                {doubledVideos.map((video, index) => (
                  <HeroVideoCard key={`${video.id}-${index}`} video={video} register={registerVideo} onReady={handleVideoReady} />
                ))}
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center items-center mb-8">
            <button onClick={onAuthRequest} className="group bg-gradient-to-r from-blue-500 to-purple-600 text-white px-10 py-5 rounded-2xl font-bold text-xl transition-all duration-300 hover:scale-105 shadow-2xl hover:shadow-blue-500/25">AI動画素材一覧を見る</button>
            <button onClick={onPurchaseRequest || onAuthRequest} className="bg-gradient-to-r from-green-500 to-emerald-600 text-white px-10 py-5 rounded-2xl font-bold text-xl transition-all duration-300 hover:scale-105 shadow-2xl hover:shadow-green-500/25">今すぐ利用する</button>
          </div>
        </div>
      </div>
    </section>
