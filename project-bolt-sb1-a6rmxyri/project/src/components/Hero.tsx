...
const Hero: React.FC<HeroProps> = ({ onAuthRequest, onPurchaseRequest }) => {
  ...
  useEffect(() => {
    ...
    const tick = (timestamp: number) => {
      ...
      if (!isReadyRef.current) {
        // Ready迥ｶ諷九′螟峨ｏ縺｣縺溘→縺阪・縺ｿ繝ｪ繧ｻ繝・ヨ
        if (wasReady) {
          lastTimestamp = null;
          wasReady = false;
        }
      ...
      // Ready迥ｶ諷九↓縺ｪ縺｣縺溽峩蠕後・繧ｿ繧､繝繧ｹ繧ｿ繝ｳ繝励ｒ繝ｪ繧ｻ繝・ヨ
      if (!wasReady) {
        lastTimestamp = null;
        wasReady = true;
      }
      ...
      if (maxShift > el.clientWidth) {
        ...
        // delta縺檎焚蟶ｸ縺ｫ螟ｧ縺阪＞蝣ｴ蜷茨ｼ医ち繝門・繧頑崛縺医↑縺ｩ・峨・繧ｹ繧ｭ繝・・
        if (delta < 100) {
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center">
          <h1 className="text-4xl sm:text-6xl lg:text-7xl font-black text-white mb-6 leading-tight">
            螳溷・邏壹・
            <span className="bg-gradient-to-r from-blue-400 to-purple-600 bg-clip-text text-transparent"> AI蜍慕判邏譚・/span>
            縺ｧ<br />
            繝槭・繧ｱ繝・ぅ繝ｳ繧ｰ繧・            <span className="bg-gradient-to-r from-purple-400 to-pink-500 bg-clip-text text-transparent"> 蜉騾・/span>
          </h1>

          <p className="text-xl sm:text-2xl text-gray-300 max-w-4xl mx-auto mb-8 leading-relaxed">
            譛磯｡・,800蜀・〒螳溷・邏壹・鬮伜刀雉ｪAI蜍慕判邏譚舌′謇九↓蜈･繧・            <br className="hidden sm:block" />
            繝｢繝・Ν繝ｻ謦ｮ蠖ｱ荳崎ｦ√ょ叉繝繧ｦ繝ｳ繝ｭ繝ｼ繝峨〒蝠・畑蛻ｩ逕ｨOK
          </p>
