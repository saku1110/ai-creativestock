...
export const findDashboardReviewRawTags = (identifier?: string): string[] | undefined => {
  const reviewKey = resolveReviewKey(identifier);
  if (!reviewKey) return undefined;
  return DASHBOARD_REVIEW_HASHTAGS[reviewKey];
};

const manifestOriginalLookup = new Map<string, string>();
for (const [key, meta] of Object.entries(remoteManifest)) {
  const url = (meta as { url?: string })?.url;
  if (!url) continue;
  const fileName = key.split('/').pop() ?? key;
  const normalized = stripWatermarkToken(fileName.toLowerCase().replace(/\.[^/.]+$/, ''));
  if (normalized) {
    manifestOriginalLookup.set(normalized, url);
  }
}
...
export const findOriginalBackupUrl = (identifier?: string): string | undefined => {
  if (!identifier) return undefined;
  const reviewKey = resolveReviewKey(identifier);
  const altKey = stripWatermarkToken(
    (identifier.replace(/\\/g, '/').split('/').pop() ?? identifier).replace(/\.[^/.]+$/, '').toLowerCase()
  );
  const candidates = [reviewKey, altKey].filter(Boolean) as string[];
  for (const key of candidates) {
    const hit = manifestOriginalLookup.get(key);
    if (hit) return hit;
  }
  return undefined;
};
